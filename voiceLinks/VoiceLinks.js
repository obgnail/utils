...
// @name        VoiceLinks
// @namespace   Sanya
// @description Makes RJ codes more useful,supports 8 bit RJ codes
...
(function () {
    ...
    const Parser = {
        ...
        wrapRJCode: function (rjCode) {
            ...
            e.addEventListener("dblclick", Popup.toASMRONE)
            return e;
        },
        ...
        rebindEvents: function (elem) {
            if (elem.nodeName === "A") {
                ...
                elem.addEventListener("dblclick", Popup.toASMRONE)
            }
            else {
                ...
                for (var i = 0, ii = voicelinks.length; i < ii; i++) {
                    ...
                    voicelink.addEventListener("click", Popup.open)
                    voicelink.addEventListener("dblclick", Popup.toASMRONE)
                }
            }
        },
    ...
    const Popup = {
        ...
        open: function(e) {
            ...
            Popup._time = setTimeout(function () {
                const code = e.target.getAttribute(RJCODE_ATTRIBUTE);
                let url = `https://www.dlsite.com/maniax/work/=/product_id/${code}.html`
                window.open(url, "_blank");
            }, CLICK_TIMEOUT)
        },

        toASMRONE: function(e) {
            clearTimeout(Popup._time);
                const code = e.target.getAttribute(RJCODE_ATTRIBUTE);
                let url = `https://www.asmr.one/work/${code}`
                window.open(url, "_blank");
        }
    }
    ...
    const DLsite = {
        parseWorkDOM: function (dom, rj) {
            ...
            if (rj.slice(-3) == "000")
                ...
                rj_group = (parseInt(rj.slice(2, -3)) + 1).toString() + "000";
                let zeros = '';
                for (var i = 0;i<(rj.length-2);i++) zeros += "0";
                rj_group = "RJ" + (zeros + rj_group).substring(rj_group.length);
            }
